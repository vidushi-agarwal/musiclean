...
class lyric_class:
    ...
    def img_wrt(self, text_arr, song_title, artist,font,line_height):
        # font_hin = ImageFont.truetype('/usr/share/fonts/truetype/Gargi/Gargi.ttf',12) #absolute path
        # font_eng_bd=ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf',10) #absolute path
        # font_eng_bd_title=ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf',25) #absolute path
        # font_eng=ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf',12) #absolute path
        ...
        # source_img = Image.open('handwash.png') #absolute path
        ...
        for i in range(4):
            for j in range(3):
                print(text_arr[ctr])
                lines = self.text_wrap(text_arr[ctr], font, box_x - 10)
                y1 = y
                for line in lines:
                    draw.text((x, y1), line, fill=(0, 0, 0), font=font)
                    y1 = y1 + line_height
                x += box_x + 30
                ctr += 1
            x = 30
        ...
        for line in lines:
            ...
            y += line_height + 30
        # source_img.save('a.png') #absolute path
    ...
    def get_lyrics(self):
        ...
        with open(settings.BASE_DIR + '/handclean_app/static/handclean_app/myauth.json') as json_file:
            ...
            apikey = data["authkey"]
        # apikey="d088aa58b583594776d4cd0acca539b2"
        ...
        try:
            ...
            if(text_arr[0]=='en'):
                font=font_eng
                line_height=font_eng.getsize('yf')[1]
            else:
                font=font_hin
                line_height = font.getsize('ब्रेकिंगन्')[1]

            self.img_wrt(text_arr,song_title,artist,font,line_height)
            return text_arr
